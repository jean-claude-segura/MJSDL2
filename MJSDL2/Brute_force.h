...
)
{
	...
	{
		const auto failure = CheckIfLockedFromMove(vLogicalBoard, mIndexToRemovedTile, arrGlobalOccurences, vMove) && vNewMoves.empty();
		const auto hashvalidation = !stopNowParallel(mIndexToTile
#ifdef _DEBUG
			, positions
#endif
		);
		if (!stopSolverNow && !failure && hashvalidation)
		{
...
inline bool CheckIfLockedFromMove(const std::vector<TileAndIndex>& vLogicalBoard, const std::map<int, Tile>& mIndexToRemovedTile, const std::map<int, int>& arrGlobalOccurences, const std::vector<int>& vMove)
{
	...
	// In case we removed the last ones.
	...
	// Vertical lock by master padlock.
	if ((c1.Index == 0x8F) || (c2.Index == 0x8F)) // For the master padlock, it's another story.
	{
		if (
			(c1.Index == 0x8F && 5 <= c2.X && c2.X <= 6 && 3 <= c2.Y && c2.Y <= 4) ||
			(c2.Index == 0x8F && 5 <= c1.X && c1.X <= 6 && 3 <= c1.Y && c1.Y <= 4)
			)
		{
			// You have to check 4 columns.
			for (int x = 5; x < 7; ++x)
				for (int y = 3; y < 5; ++y)
					for (int z = 0; z < 4; ++z)
						if (pairing == arrBoard[x][y][z].TileObject.Pairing)
							return true;
		}
	}

	// Crossed vertical lock.
	if ((c1.Index != 0x8C) && (c1.Index != 0x8D) && (c1.Index != 0x8E) && // There is nothing under or above the main padlock
		(c2.Index != 0x8C) && (c2.Index != 0x8D) && (c2.Index != 0x8E)) // There is nothing under or above the main padlock
	{
		if ((c1.Index != 0x8F) && // For the master padlock, it's another story.
			(c2.Index != 0x8F)) // For the master padlock, it's another story.
		{
		...
		else if (c1.Index == 0x8F) // For the master padlock, it's another story.
		{
			// You have to check 4 columns.
			for (int x = 5; x < 7; ++x)
				for (int y = 3; y < 5; ++y)
					for (int z = 0; z < 4; ++z)
					{
						const auto& item = arrBoard[x][y][z];
						const auto count = arrGlobalOccurences.find(item.TileObject.Pairing)->second;
						if (count == 2)
							for (int z = c2.Z + 1; z < 4; ++z)
								if (arrBoard[c2.X][c2.Y][z].TileObject.Pairing == item.TileObject.Pairing) // If one tile under c1 has a counterpart above c2, it's over.
		...
		else if (c2.Index == 0x8F) // For the master padlock, it's another story.
		{
			// You have to check 4 columns.
			for (int x = 5; x < 7; ++x)
				for (int y = 3; y < 5; ++y)
	...
	if (c1.Y == c2.Y && c1.Z == c2.Z) // Same height, same row
	{
		...
		for (const auto& tileAndIndex : vLogicalBoard)
		{
			if (tileAndIndex.Y == c1.Y && tileAndIndex.Z == c1.Z) // Same height, same row
			{
				if (beginning == -1)
	...
	const std::map<int, Tile>& mIndexToTile)
{
	...
	for (const auto& index : vMove)
	{
		...
		{
			const auto& temp = arrIndexToBoardCoord[index];
...
{
	std::map<int, int> arrGlobalOccurences {};
	for (const auto& tileAndIndex : plateau.getLogicalBoard())
		++arrGlobalOccurences[tileAndIndex.TileObject.Pairing];
	...
	while (!vMoves.empty())
	{
		...
		do
		{
			...
			for (auto itMove = vMoves.begin(); itMove != vMoves.end(); )
			{
				const auto& vMove = *itMove;
				const auto index = *vMove.begin();
				const auto pairing = plateau.getTilesMap().find(index)->second.Pairing;
				if (vMove.size() == arrGlobalOccurences.find(pairing)->second)
				{
					arrGlobalOccurences.erase(pairing);
					loop = true;
					plateau.RemovePairOfTiles(vMove[0], vMove[1]);
					vSolution.emplace_back(std::make_pair(vMove[0], vMove[1]));
					if (vMove.size() == 4)
					{
						plateau.RemovePairOfTiles(vMove[2], vMove[3]);
						vSolution.emplace_back(std::make_pair(vMove[2], vMove[3]));
					}
			...
			vMoves.clear();
			SetMoves(plateau.getLogicalBoard(), plateau.getRemovable(), vMoves);
...
inline bool tryRandomHeuristicsMostBlocked(Board plateau, std::vector<std::pair<int, int>>& vSolution)
{
	std::array<std::array<std::array<TileAndIndex, 4>, 8>, 12> arrBoard = {};
	for (const auto& tileAndIndex : plateau.getLogicalBoard())
	{
	...
	for(int z = 0; z < 4; ++z)
		for (int y = 0; y < 8; ++y)
		{
			...
			{
				if (!plateau.getRemovableFromIndex(arrBoard[x][y][z].Index))
				{
					int blockersCount = 0;
					if (x > 0 && arrBoard[x - 1][y][z].Index != -1)
						++blockersCount;
					if (x < 11 && arrBoard[x + 1][y][z].Index != -1)
						++blockersCount;
					if (y > 0 && arrBoard[x][y - 1][z].Index != -1)
						++blockersCount;
					if (y < 7 && arrBoard[x][y + 1][z].Index != -1)
						++blockersCount;
					if (z > 0 && arrBoard[x][y][z - 1].Index != -1)
						++blockersCount;
					if (y < 3 && arrBoard[x][y][z + 1].Index != -1)
						++blockersCount;
					mostBlocked[arrBoard[x][y][z].TileObject.Rank] = std::max(mostBlocked[arrBoard[x][y][z].TileObject.Rank], blockersCount);
				}
			}
		}
	
	mostBlocked[plateau.getTilesMap().find(0x8D)->second.Pairing] += 3;
	...
	while (!vMoves.empty())
	{
		...
		if (!vMoves.empty())
		{
			...
			for (const auto& move : vMoves)
			{
				const auto rank = mIndexToTile.find(move.front())->second.Rank;
				const auto blockingValue = mostBlocked[rank];
				vSortedMoves.emplace_back(std::make_pair(move, blockingValue));
			}
...
inline bool testAll(const Board& plateau, std::array <unsigned int, 10>& arrResult)
{
	std::vector<std::pair<bool (*)(Board plateau, std::vector<std::pair<int, int>>& vSolution), std::string>> vTries;
	...
	vTries.push_back({ tryBruteForceOrderingFreedPadlocksFirst, "tryBruteForceOrderingFreedPadlocksFirst" });
	vTries.push_back({ tryMaxBlock, "tryMaxBlock" });
	vTries.push_back({ tryRandomHeuristicsMostBlocked, "tryRandomHeuristicsMostBlocked" });
